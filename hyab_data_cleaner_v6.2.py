...
LTM Trend (rolling 12-month totals): 
{chart_data['ltm_trend_summary']}

Monthly Sales (actual monthly revenue, NOT LTM - use for seasonality analysis):
{chart_data.get('monthly_sales_summary', 'Not available')}
...
{{
    ...
    "ltm_trend": "Analysis of the ROLLING LTM (trailing 12-month) LINE CHART - discuss the overall trajectory, inflection points, and what the trend indicates about business momentum",
    "monthly_sales": "Is revenue growing, stable, or declining? Quarterly/annual trajectory. Seasonality patterns (e.g. July shutdown).",
    "yoy_comparison": "Year-over-year comparison - how does current year compare to prior year",
    "decomposition": "What's driving revenue changes - churn vs growth vs new customers",
    "churn_timeline": "When customers are leaving - patterns in the churn timeline chart",
    "revenue_bridge": "Revenue bridge analysis - the flow from prior year to current year",
    "cohorts": "Customer cohort health - churned, declining, growing, new segments",
    ...
    "at_risk": "At-risk customers (declining 3+ months) with specific retention actions",
    "top_customers": "Portfolio concentration and health of top 20 customers",
    "strategic_recommendations": "5-6 prioritized actions: (1) PROTECT growing accounts (NIBE, Linak, Eurosign, Tetra Pak, Atlas Copco), (2) STOP DECLINE in EMS sector (Zollner, Kitron, AMADA), (3) WIN-BACK Metso (1M SEK - they WERE a customer), (4) INVESTIGATE automotive weakness, (5) Growth opportunities in strong sectors"
}}"""
        ...
        if f:
            try:
                ...
                if curr and prev:
                    ...
                    # Monthly sales summary (last 12 months actual monthly sales, NOT LTM)
                    months = sorted(data['monthly_totals'].keys())[-12:]
                    def format_month(m):
                        try:
                            from datetime import datetime
                            dt = datetime.strptime(m, '%Y-%m')
                            ...
                            return m
                    monthly_sales_summary = "\n".join([f"- {format_month(m)}: {data['monthly_totals'].get(m, 0):,.0f} SEK" for m in months])
