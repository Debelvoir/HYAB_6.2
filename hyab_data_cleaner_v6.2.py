...
{{
    ...
    "ltm_trend": "LTM trend analysis",
    "monthly_sales": "Monthly pattern observations", 
    "yoy_comparison": "YoY performance analysis",
    "decomposition": "What's driving revenue changes",
    "churn_timeline": "When/why customers leaving",
    "revenue_bridge": "Revenue movement analysis",
    "cohorts": "Customer cohort health",
    "industry_analysis": "Which of HYAB's ACTUAL sectors are performing well vs struggling",
    "at_risk": "At-risk customers with specific retention actions",
    "top_customers": "Portfolio concentration and health analysis",
    "strategic_recommendations": "5-6 prioritized actions: (1) PROTECT growing accounts (NIBE, Linak, Eurosign, Tetra Pak, Atlas Copco), (2) STOP DECLINE in EMS sector (Zollner, Kitron, AMADA), (3) WIN-BACK Metso (1M SEK opportunity - they WERE a customer), (4) INVESTIGATE why automotive is weak despite having Volvo/Scania, (5) Any genuine new business opportunities based on current sector strengths"
}}"""
    ...
    try:
        response = requests.post(
            ...
            json={
                "model": "claude-opus-4-5-20251101",  # Opus 4.5 for deeper strategic analysis
                "max_tokens": 4000,  # Increased for richer insights
                "messages": [{"role": "user", "content": prompt}]
            },
            timeout=90  # Increased timeout for more thorough analysis
        )
...
def generate_html(data, curr_ltm, prev_ltm, commentary=None):
    ...
    def format_strategic_recs(text):
        if not text:
            return ''
        import re
...
<div class="chart-section"><h2>⚠️ Churn Timeline: When Customers Left</h2><p style="color:#94A3B8;font-size:0.85rem;margin:-8px 0 16px 0;">Lost LTM revenue by last order month — shows WHEN churned customers stopped ordering</p><div class="chart-wrapper"><canvas id="churnTimelineChart"></canvas></div>{commentary_block('churn_timeline')}</div>
